...
from core import SM, LG, cc, fb, ps_sql, ps_nlq, rs, TP_CR, TP_SY, TP_RT

_dk = os.getenv("DASHSCOPE_API_KEY")
_cl = _OAI(api_key=_dk, base_url="https://dashscope.aliyuncs.com/compatible-mode/v1", timeout=600)
...
def cl(pr, md):
    ...
    try:
        rp = _cl.chat.completions.create(model=md, messages=[{"role": "user", "content": pr}], stream=True)
        tx = ""; t0 = time.time(); la = t0
        for ck in rp:
            nw = time.time()
            if nw - t0 > 560 or nw - la > 120:
                LG.warning("LLM %s stream timeout (total=%.0fs idle=%.0fs)", md, nw - t0, nw - la); break
            la = nw
...
def _dg(sc, bd, lo, hi):
    ln = []
    if sc > hi:
        tp = sorted([(k, v) for k, v in bd.items() if v > 0], key=lambda x: x[1], reverse=True)[:3]
...
def wk(wi, st, tt):
    ...
    so, qo, sc, bd = None, None, -1, {}
    for at in range(1, MR + 1):
        if st.azc(): return False
        cr = TP_CR.format(lo=lo, hi=hi)
        if at == 1:
            pr = TP_SY.format(lo=lo, hi=hi, complexity_rules=cr, schema_str=sc_['formatted_schema'],
                              comp_guide=gs, underused_tables=us, few_shot=fs)
        else:
            pr = TP_RT.format(attempt=at, max_attempts=MR, prev_score=sc, lo=lo, hi=hi,
                              breakdown=fb(sc, bd), diagnosis=_dg(sc, bd, lo, hi), complexity_rules=cr,
                              schema_str=sc_['formatted_schema'], comp_guide=gs, underused_tables=us)
        rw = cl(pr, MSY)
        ...
        sc, bd = cc(so)
        LG.info("W-%03d attempt %d: score=%d target=[%d,%d]  %s", wi, at, sc, lo, hi, fb(sc, bd))
        if lo <= sc <= hi: break

    if not so or sc < 0 or not (lo <= sc <= hi):
        LG.info("W-%03d FAIL after %d attempts (score=%s)", wi, MR, sc); return False

    rv = rs(dp, so)
    if rv is None:
        LG.info("W-%03d SQL exec failed/timeout -> sef", wi)
        st.sef(tt, di, qo, so, sc, bd); return False
    ...
    if not ok:
        ...
        st.sdf(tt, di, qo, so, sc, bd); return False
    ...
    if cnq:
        if len(cnq) == 1: qo = cnq[0]
            ...
            try:
                ...
                sm = torch.nn.functional.cosine_similarity(re_.unsqueeze(0), re_.unsqueeze(1), dim=2)
                sm.fill_diagonal_(0)
                av = sm.sum(dim=1) / (len(cnq) - 1)
                bi = av.argmax().item(); qo = cnq[bi]
                LG.info("W-%03d reverse: %d candidates, picked #%d (avg_sim=%.3f)", wi, len(cnq), bi, av[bi].item())
            except Exception as e:
                LG.warning("W-%03d reverse similarity error: %s", wi, e); qo = cnq[0]

    ok = st.svr(tt, di, qo, so, sc, bd, em, sk)
    LG.info("W-%03d %s tier=%s total=%d", wi, "SAVED" if ok else "skip", tt, st.ccn(tt))
    return ok
